  ...
  _.each(data , function(value){
    ...
    if ((value.username === selectedUser || selectedUser === 'everyone')){
      $('#tweets').append(Chat.Templates.messageItem({
        ...
        message: value.content.slice(value.content.indexOf(':') + 1),
...
var submitTweet = function(message, callBack){
  $.ajax({
    ...
    url: 'http://localhost:8081/classes/' + selectedRoom + '/',
    type: "POST",
...
var updatePage = function(){
  // get all of the data
  $.ajax('http://localhost:8081/classes/' + selectedRoom + '/', {
